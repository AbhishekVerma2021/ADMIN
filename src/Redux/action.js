...
export const getCalls = () => {
  return (dispatch) => {
    dispatch({ type: GET_BUISNESS_CALLS_PENDING });
    Axios.get('/admin/getBusinessCalls')
    .then(res => dispatch({ type: GET_BUISNESS_CALLS_FULFILLED, payload: res.data }))
    .catch(er => dispatch({ type: GET_BUISNESS_CALLS_REJECTED, payload: er }));
  };
};
...
export const getLine1Data = () => {
  return (dispatch) => {
    dispatch({ type: GET_LINE_ONE_DATA_PENDING });
    Axios.get('/admin/monthlyusers')
      .then(res => {
        dispatch({ type: GET_LINE_ONE_DATA_FULFILLED, payload: res.data });
      })
      .catch(err => dispatch({ type: GET_LINE_ONE_DATA_REJECTED, payload: err }));
  };
};

export const getBarGraphData = () => {
  return (dispatch) => {
    dispatch({ type: GET_BAR_GRAPH_DATA_PENDING });
    Axios.get('/admin/weeklyusers')
      .then(res => dispatch({ type: GET_BAR_GRAPH_DATA_FULFILLED, payload: res.data }))
      .catch(err => dispatch({ type: GET_BAR_GRAPH_DATA_REJECTED, payload: err }));
  };
};
